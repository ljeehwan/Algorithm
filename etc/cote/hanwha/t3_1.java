		...
		for(int i = 0 ; i < len-1 ; i++) {
			...
			if(status == 5) {
				...
				if(first[0].equals(second[0])) {
					...
					if(num1 + 1 == num2) { // 금토
						//금토일인가?
						String[] temp = login_dates[i+2].split("/");
						num2 = Integer.parseInt(temp[1]);
						if(num1 + 2 == num2) {
							//금토일월
							String[] temp2 = login_dates[i+3].split("/");
							if(num1 + 3 == num2) {
								//금토일월
								cnt++;
								i = i+2;
				...
				if(status == 6 || status == 7 || nextStatus == 6 || nextStatus == 7) {
					continue;
				}
				else {
					first = login_dates[i].split("/");
